...
const VideoChat = () => {
            ...
            appPeer.on("open", (id) => {
                if (is_caller) {
                    onCallPeer(appPeer, {
                        video: videoConfig,
                        audio: audioConfig
                    }, callIds, localRef.current, remoteRef.current)
                }
                if (!is_caller) {

                    onAnswerPeer(appPeer, {
                        video: videoConfig,
                        audio: audioConfig
                    }, callIds, localRef.current, remoteRef.current)
                }
    ...
    return (
        <div className="position-relative vw-100 vh-100 bg-dark">
            <video className="position-absolute remote-stream w-100 h-100" ref={remoteRef}></video>
            <video className="position-absolute local-stream w-25 h-25 rounded-1rem"
                   ref={localRef}></video>
