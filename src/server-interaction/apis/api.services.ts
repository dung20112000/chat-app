...
const isApiMatch = (url: string, method: Method, testArray: IMethodUrlObject[]) => {
    return testArray.findIndex((api) => api.url === url && api.method.toLowerCase() === method.toLowerCase());
}
export const callApi = (
    ...
    if(isApiMatch(url, method, noLoadingApis) < 0 && !url.includes("/conversations/")){
